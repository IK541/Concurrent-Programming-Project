...
void *publisher(void *arg) {
	...
	while (1) {
		i = (i + 1) % PUBLICATIONS;
		if (TQueuePut(tqueue, arr[i * PUBLISHERS + id]) == -1)
			break;
		...
		if (TQueueRemoveMsg(tqueue, arr[rand_num]) == -1)
			break;
		printf("> thread %d removed: %d\n", id, *arr[i]);
		if (!(rand_num % 16)) {
			rand_num += 1;
			if (TQueueSetSize(tqueue, (int *)&rand_num) == -1)
				break;
		}
	}
...
void *subscriber(void *arg) {
	...
	while (1) {
		...
		if (!(n % 16)) {
			if (TQueueUnsubscribe(tqueue, &this_thread) == -1)
				break;
			if (TQueueSubscribe(tqueue, &this_thread) == -1)
				break;
		}
...
int main() {
	...
	publisher_data **pub_data_arr =
		malloc(PUBLISHERS * (sizeof(publisher_data *)));
	...
	subscriber_data **sub_data_arr =
		malloc(SUBSCRIBERS * (sizeof(subscriber_data *)));
